<script>
    ...
    export let gameStore;   // 接收 store 对象

    $: storeValue = $gameStore;   // 自动解包
    $: currentGrid = storeValue.grid;
    $: invalidCells = storeValue.invalidCells;
    $: gameWon = storeValue.won;
    ...
    function getValueAtCursor(grid, cursorStore) {
        if (cursorStore.x === null || cursorStore.y === null) return null;
        return grid[cursorStore.y][cursorStore.x];
    }
</script>
            ...
            {#each currentGrid as row, y}
                {#each row as value, x}
                    <Cell
                            ...
                            userNumber={!gameStore.isFixed(y, x)}
                            sameArea={$settings.highlightCells && !isSelected($cursor, x, y) && isSameArea($cursor, x, y)}
                            sameNumber={$settings.highlightSame && value && !isSelected($cursor, x, y) && getValueAtCursor(currentGrid, $cursor) === value}
                            conflictingNumber={$settings.highlightConflicting && !gameStore.isFixed(y, x) && invalidCells.includes(x+','+y)}
                    />
                {/each}
            {/each}
        </div>
